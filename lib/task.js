...
module.exports = options => callback => {
    ...
    let { taskName , src , dest , ext , folder } = {
    ...
    .pipe( through2( 
        async function ( file, enc, cb ) {
            ...
            console.log(`\n${taskName} convert: "${file.basename.green}"`)
            required.push(`//@require ${`"${file.stem}"`}`)
            // ttf info parser
            ...
            */
            ...
            |         url('${folder}/${file.stem}/${file.stem}.woff2') format('woff2'),    /* Super Modern Browsers */
            |         url('${folder}/${file.stem}/${file.stem}.woff')  format('woff'),     /* Modern Browsers       */
            |         url('${folder}/${file.stem}/${file.stem}.ttf')   format('truetype'); /* Safari, Android, iOS  */
            |}`.replace(/\s+?\|/g,"\n")
        ...
        function( cb ){
            console.log(`\n${taskName} ${`${dest}/index.styl`.green}\n`)
            fs.appendFileSync(`${dest}/index.styl`,"\n"+required.join("\n"))
            cb()
        }
    ))
    .pipe( gulp.dest(`${dest}`) )
